...
- **`cbsecurity.firewall.handlerAnnotationSecurity` must stay `false`.** Its scanner calls `handlerService.getHandlerBean( currentEvent )` from `preProcess`. On any unmatched URL that resolves the *invalid* event and sets ColdBox's `request._lastInvalidEvent` recursion guard; ColdBox then dispatches for real, calls `invalidEvent()` a second time, sees the guard and throws `HandlerService.InvalidEventHandlerException`. Net effect: **every 404 becomes a 500**, site-wide. cbsecurity has a guard for this but computes it as `coldboxVersion <= 5`, so it is inert on ColdBox 8. Safe to disable only because no handler here carries `@secured`; re-enabling means also fixing the double resolution (an explicit catch-all route to `page.show`, so no event is ever "invalid"). Full write-up in the config comment at `config/Coldbox.bx`.
- **`ENVIRONMENT` barely selects anything — don't reason from it.** ColdBox's `detectEnvironment()` defaults `environment` to `"production"`, and this app defines neither a `detectEnvironment()` method nor an `environments` struct. The var works only because `ApplicationLoader.cfc` passes `getSystemSetting("ENVIRONMENT","")` in. So an unset/typo'd value silently means production, and `development()` never runs. Do not assume a dev/prod behavioural difference comes from these blocks without checking the setting at runtime.
- **A bug can be production-only on byte-identical code.** The 404→500 bug above never reproduced on the dev stack. When that happens, stop guessing and read ColdBox's own error payload: `HandlerService` logs `REGISTEREDHANDLERS`, `FULLEVENT`, `EVENT`, `REQUESTEVENT`, `ROUTEDURL`, `ROUTE` and a full `CALLSTACK` as LogBox `extra` data. The callstack named the culprit interceptor in one line after four wrong hypotheses (env overrides, ColdBox version, dev-only modules, request-scope leakage).
...
- **Defining `onMount()` on a wire disables the params-to-data auto-copy.** CBWIRE's default behavior is to copy every `wire("foo", { postId: 42 })` param into `variables.data` if there's no `onMount`. As soon as you declare `onMount`, that copy is skipped and you have to do it yourself: `function onMount( event, rc, prc, params ) { if ( structKeyExists( arguments.params, "postId" ) ) data.postId = arguments.params.postId; }`. Symptom: every `bx:if (args.postId ?: 0) > 0` in your template renders false even though you passed postId via wire().
- **All wires currently live in the app-root `wires/` folder — even the admin-only ones.** This is a *workaround for a CBWIRE bug*, not the intended long-term layout. CBWIRE's `@ModuleName` suffix (`wire("posts.editor@admin")`) resolves single-file components correctly but fails on two-file components (`.bx` class + separate `.bxm` template) when they live in `modules_app/{name}/wires/`. To keep Phase 2 unblocked, every wire is single-file in app-root `wires/` and called with a bare name. When a patched CBWIRE ships, migrate to `modules_app/admin/wires/` and switch the call sites to `wire("name@admin")`. Clear `modules/cbwire/views/tmp/` after the move.
